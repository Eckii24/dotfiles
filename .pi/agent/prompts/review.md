...
6. Run one independent formal review pass with `code-reviewer`, passing the focus explicitly, unless the change is low-risk non-code work where an inline checklist is enough.
7. Do **not** auto-fix findings.
8. Only create/update `.ai/<slug>-review.md` when the task is tracked or the user explicitly wants a review artifact.
