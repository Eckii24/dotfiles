...
export interface UsageEvent {
	...
	provider: string; model: string; input: number; cacheRead: number; cacheWrite: number; output: number; reasoning: number; totalTokens: number; costInput: number; costCacheRead: number; costCacheWrite: number; costOutput: number; cost: number;
...
export async function readSessionUsage(root: string): Promise<ReadUsageResult> {
	...
	for (const session of sessions) {
		...
		const visit = (message: RecordValue, fallbackTimestampMs: number | undefined, agentKind: AgentKind, agentName: string, subagentDepth: number): void => {
			...
			if (message.role === "assistant" && record(message.usage)) { const usage = message.usage; const recordedCost = record(usage.cost) ? usage.cost : undefined; const cost = number(recordedCost?.total); const costInput = number(recordedCost?.input), costCacheRead = number(recordedCost?.cacheRead), costCacheWrite = number(recordedCost?.cacheWrite), costOutput = number(recordedCost?.output); const input = number(usage.input), cacheRead = number(usage.cacheRead), cacheWrite = number(usage.cacheWrite), output = number(usage.output); events.push({ sessionId: session.sessionId, sessionTitle: session.sessionTitle, threadId: thread.sessionId, threadTitle: thread.sessionTitle, sessionPath: session.sessionPath, timestampMs: at, project: session.project, workflow: session.workflow, provider: typeof message.provider === "string" ? message.provider : "unknown", model: typeof message.model === "string" ? message.model : "unknown", input, cacheRead, cacheWrite, output, reasoning: number(usage.reasoning), totalTokens: number(usage.totalTokens) || input + cacheRead + cacheWrite + output, costInput, costCacheRead, costCacheWrite, costOutput, cost, agentKind, agentName, subagentDepth }); return; }
