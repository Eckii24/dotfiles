...
test.skipIf(!g19Live)("G19 foreign cleanup: owned close preserves foreign pane and tab", async () => {
	...
	try {
		...
		const started = await observer.splitPane({ direction: "right", targetPaneId: details.children[0].paneId, workspaceId: details.workspaceId, cwd: process.cwd(), focus: false }); foreignPaneId = nestedPaneId(started); expect(foreignPaneId).toBeTruthy();
