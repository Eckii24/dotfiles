	...
	return { registry, async execute(raw: unknown, ctx: RuntimeContext, signal?: AbortSignal, onUpdate?: ToolUpdate): Promise<any> {
		...
		try {
			...
			const run = async (entry: PreparedLeaf, previous?: string): Promise<LifecycleResult> => {
				...
				const envelope = JSON.stringify({ type: "herdr_subagent_task", rootRunId: ids.rootRunId, leafRunId: entry.ids.leafRunId, turnId: entry.ids.turnId, task });
				let life: LifecycleResult;
				try { life = await (deps.runLifecycle ?? runLifecycleTurn)(lifecyclePort(client!, entry.leaf.paneId), sessionPort(deps.sessionRoot ?? sessionRoot), { agentId: entry.leaf.paneId, task: envelope, turnId: entry.ids.turnId, timeoutMs: input.timeoutSeconds * 1000, clock: { now }, sleeper: { sleep: async ms => await new Promise(resolve => setTimeout(resolve, ms)) }, signal, onReady: async () => { await entry.launch.cleanupAfterReady(); entry.leaf.status = "working"; registry.updateLeaf(ids.rootRunId, entry.ids.leafRunId, { status: "working" }); } }); }
				...
				registry.updateLeaf(ids.rootRunId, entry.ids.leafRunId, { status: entry.leaf.status, activeTurnId: active ? entry.ids.turnId : undefined, activeMarker: active ? envelope : undefined, ...(entry.leaf.piSession ? { session: { source: "herdr:pi", path: entry.leaf.piSession.path, sessionId: entry.leaf.piSession.sessionId } } : {}) });
				return life;
			};
			if (input.mode === "parallel") {
