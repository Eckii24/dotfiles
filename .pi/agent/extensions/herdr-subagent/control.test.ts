...
test("follow_up uses same injected lifecycle port, waits native final, and retains trusted session", async () => {
 ...
 const runtime = createHerdrSubagentControlRuntime({ registry, createClient: () => client, preflight: async () => ({ socketPath: "/socket" }), sessionRoot: root, lifecyclePort: value => { expect(value).toBe(client); return port; }, sessionPort: value => { expect(value).toBe(root); return sessions; }, runLifecycle: (async (actualPort, actualSessions, input) => { expect(actualPort).toBe(port); expect(actualSessions).toBe(sessions); expect(input.retainedDone?.sessionId).toBe("session"); expect(input.task).toContain(input.turnId); calls.push("lifecycle"); lifecycleStarted(); await gate; return { status: "succeeded", state: "done", delivered: true, enterSent: true, session: { source: "herdr:pi", kind: "path", root, path: trustedPath, sessionId: "session", bytes: 1 }, result: { pending: false, status: "succeeded", output: "FINAL", stopReason: "stop", sessionId: "session", anchorEntryId: "anchor", finalEntryId: "final" } }; }) as any });
