function update-music() {
    ...
    emulate -L zsh
    # For advanced globbing features if needed.
    # setopt extendedglob

    # Local variables for storing parsed arguments and other data.
    local album_name="" # Initialize as empty, making it optional
    local music_dir
    ...
    _usage() {
        ...
        echo "Usage: ${FUNCNAME[1]} --path \"/path/to/music\" [--album \"Album Title\"]" >&2
        echo "       ${FUNCNAME[1]} -p \"/path/to/music\" [-a \"Album Title\"]" >&2
        echo "" >&2
        echo "Options:" >&2
        echo "  -a, --album <name>    Optional. Set the album name for all tracks." >&2
    ...
    fi # No 'else' block needed as album_name is optional

    # --- Retrieve and validate music directory path (required) ---
    ...
    if [[ -n "$album_name" ]]; then
        ...
        echo "No album name specified. Album tag will not be explicitly set by this script."
    fi
    echo "Processing files in directory: \"$music_dir\""

    # --- Find music files in the specified directory ---
    # Glob qualifiers: (N) Nullglob, (.) Regular files, (od) Order by name.
    ...
    for file_path in "${files_to_process[@]}"; do
        ...
        # Prepare the eyeD3 command arguments
        local eyeD3_command_args=()
        if [[ -n "$album_name" ]]; then
            eyeD3_command_args+=(--album "$album_name")
        fi
        eyeD3_command_args+=(--to-v2.4 "$file_path" -n "$current_index" -N "$total_files")

        # Execute eyeD3 with the constructed arguments
        if eyeD3 "${eyeD3_command_args[@]}"; then
            echo "Successfully processed: \"$file_path\""
        else
            echo "Error processing \"$file_path\". eyeD3 exited with status $?. Check eyeD3 output for details." >&2
